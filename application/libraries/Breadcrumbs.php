...
class Breadcrumbs {
  ...
  { 
    ...
    $this->restaurants_tag_open = $this->ci->config->item('restaurants_tag_open');
    $this->restaurants_tag_close = $this->ci->config->item('restaurants_tag_close');
    $this->restaurants_crumb_open = $this->ci->config->item('restaurants_crumb_open');
    $this->restaurants_crumb_close = $this->ci->config->item('restaurants_crumb_close');
    $this->restaurants_crumb_last_open = $this->ci->config->item('restaurants_crumb_last_open');
    $this->restaurants_crumb_last_close = $this->ci->config->item('restaurants_crumb_last_close');
    $this->restaurants_crumb_divider = $this->ci->config->item('restaurants_crumb_divider');
  ...
  /**
   * Generate breadcrumb restaurants
   *
   * @access  public
   * @return  string
   */   
  function show_restaurants()
  {
    if ($this->breadcrumbs) {
      ...
      // set output variable
      $output = $this->restaurants_tag_open;
      
      // construct output
      foreach ($this->breadcrumbs as $key => $crumb) {
        $keys = array_keys($this->breadcrumbs);
        if (end($keys) == $key) {
          $output .= $this->restaurants_crumb_last_open . '' . $crumb['page'] . '' . $this->restaurants_crumb_last_close;
        } else {
          $output .= $this->restaurants_crumb_open.'<a href="' . $crumb['href'] . '"><span>' . $crumb['page'] . '</span><i class="breadcrumbs_arrow sprite sprite-ico-breadcrumbs-end"></i></a> '.$this->restaurants_crumb_close;
        }
      }
      
      // return output
      return $output . $this->restaurants_tag_close . PHP_EOL;
    }
